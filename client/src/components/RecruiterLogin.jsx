    ...
    <div className='absolute top-0 bottom-0 left-0 right-0 z-10 backdrop-blur-sm  bg-black/30 flex justify-center items-center'>
      <form className='relative bg-white p-10 rounded-xl text-slate-500 '>
        ...
        <div className='border px-4 py-2 flex items-center gap-2 rounded-full mt-3'>
          ...
          <input className='outline-none text-sm' onChange={e => setName(e.target.value)} type="email" placeholder='Email id' value={email} required />
        </div>
        <div className='border px-4 py-2 flex items-center gap-2 rounded-full mt-3'>
          <img src={assets.lock_icon} alt="" />
          <input className='outline-none text-sm' onChange={e => setName(e.target.value)} type="text" placeholder='Password' value={password} required />
        </div>
        <p className='text-sm text-blue-600 mt-2 cursor-pointer'>Forgot password</p>
        </>
